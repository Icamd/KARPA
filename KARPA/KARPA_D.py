...
if __name__ == '__main__':
    ...
    if args.dataset == 'cwq':
        ...
        output_file = "../predictions/cwq/karpa_d/predictions.jsonl"
        ...
        rule_dataset = load_jsonl(args.rule_path)
        dt = merge_rule_result(dt, rule_dataset)
        output_file = '../predictions/webqsp/karpa_d/predictions.jsonl'
        fout, processed_list = get_output_file(output_file, force=False)
        id_str = 'QuestionId'
    ...
    for dataset in tqdm(datasets):
        ...
        if len(new_cluster_chain_of_entities) == 0:
            ...
            result = reasoning_h4(question, [[new_cluster_chain_of_entities]], args)
            new_result = reasoning_h4(question, [[new_cluster_chain_of_entities_backup]], args)
            results = result + new_result
            use_cot = False
            use_exhaustivity = False

            final_entities1 = [[x[2].split(', ') for x in sublist] for sublist in [[new_cluster_chain_of_entities]][-1]][0]
            final_entities1.extend([[x[2].split(', ') for x in sublist] for sublist in [[new_cluster_chain_of_entities_backup]][-1]][0])
            final_entities = []
            for final in final_entities1:
                final_entities.extend(final)
            extract_answers = []
            candidate_answers = []
            relations2 = re.findall(r'\{([^}]*)\}', results)
            for i in relations2:
                 candidate_answers.extend(i.split(', '))
            ...
            if len(extract_answers) == 0:
                for answer in final_entities:
                    for res in relations2:
                        if answer.lower() in res.lower():
